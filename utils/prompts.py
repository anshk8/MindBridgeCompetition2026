...
# ================================================================= #
# Difficulty Ranker Agent Prompts                                    #
# ================================================================= #

def buildDifficultyRankerSystemPrompt() -> str:
    """System prompt for the difficulty ranker agent."""
    return """You are an expert SQL complexity analyst. Your job is to classify a natural language database question into one of four difficulty tiers based on the SQL it would require:

DIFFICULTY TIERS:
─────────────────────────────────────────────────────────────────
EASY
  • Single table query (SELECT, WHERE, basic ORDER BY / LIMIT)
  • Simple aggregations on one table (COUNT, SUM, AVG, MIN, MAX)
  • No JOINs, no subqueries
  Examples: "How many customers are there?", "List all brands",
            "What is the most expensive product?"

MEDIUM
  • 2-table JOIN (INNER, LEFT, etc.)
  • Grouped aggregations (GROUP BY) across joined tables
  • Simple subqueries (single level, non-correlated)
...
HARD
  • 3 or more table JOINs
  • Nested or correlated subqueries
  • CTEs (WITH clauses)
  • Window functions (RANK, DENSE_RANK, ROW_NUMBER, LAG, LEAD)
  • Complex multi-step calculations (e.g. percentage of total revenue)
  • Self-joins or CROSS JOINs
  Examples: "What is the revenue breakdown by category as a percentage of total?",
            "Rank customers by total spending within each state"

AMBIGUOUS
  • Question is vague, contradictory, or cannot be answered
  • References columns/concepts that do not exist in the schema
  • Requires external knowledge the database does not contain
  • Multiple valid but conflicting interpretations
  Examples: "What is the customer satisfaction score?",
            "Who is the best employee?", "Show me recent data"
─────────────────────────────────────────────────────────────────

CLASSIFICATION PROCESS:
1. Identify which tables are needed.
2. Count the number of JOINs required.
3. Determine if subqueries, CTEs, or window functions are needed.
4. Check whether any concepts in the question are missing from the schema.
...
def buildDifficultyRankerUserPrompt(question: str, schemaContext: str) -> str:
    """
    Build the user prompt for difficulty ranking.

    Args:
        question: The natural language question to classify
        schemaContext: Formatted schema information with table/column details

    Returns:
        str: Complete user prompt for the ranker
    """
    return f"""DATABASE SCHEMA:
{schemaContext}
...
Analyse this question against the schema and assign a difficulty tier.
Think step by step: which tables are needed, how many JOINs, any subqueries/CTEs/window functions, any ambiguity?"""
