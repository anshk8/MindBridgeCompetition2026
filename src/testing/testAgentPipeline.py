...
the LangGraph pipeline:
    ...
    generateSqlNode  →  kCandidatesNode  (always — exits early on first passing result)
                     →  irrelevantNode   (intent = Irrelevant)
                     →  ambiguousNode    (intent = Ambiguous, multiConversational=False)

Each test result surfaces the full LangGraph state so you can see exactly
which path was taken and how the validator scored the final query.
"""
...
from src.testing.queriesToTest import EXTENDED_QUERIES
...
class SQLAgentTester:
    ...
    def run_test(self, test_query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Skip execution for sentinel values (irrelevant / ambiguous exits)
            if not final_sql or final_sql.startswith('-- IRRELEVANT_QUERY:') or final_sql.startswith('-- AMBIGUOUS_QUERY:'):
                result['success'] = False
                result['error'] = final_sql or 'Empty SQL returned'
                return result
...
def main():
    ...
    try:
        ...
        results = tester.run_test_suite(difficulties=['hard', 'easy', "medium"], query_bank=TEST_QUERIES)
